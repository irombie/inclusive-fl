...
* ```--dist_noniid:```  Used in non-iid setting (--iid=0). Option to assign each user proportion of each class samples according to Dirichlet distribution. Default set to 0 to omit this option. Set to 1 to select this option. Option to be selected other non-IID option for unequal splits must be set to 0 (--unequal=0).
