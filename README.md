...
* ```--dist_noniid:```  Used in non-iid setting (--iid=0). Option to give each user the proportion of each class samples according to Dirichlet distribution. Default set to 0 to omit this option. Set to 1 to select this option. Option to be selected other non-IID option for unequal splits must be set to 0 (--unequal=0).
* ```--federated_learning_method:``` Name of method to use. Currently supports "FedAvg" and "FedBN"
