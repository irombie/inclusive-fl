            ...
            for ims, labels in tqdm.tqdm(testloader):
                for i in range(self.num_classes):
                    if len(labels[labels == i]) > 0:
                        ...
                        class_distances[i] += (
                            (softmax_outputs[:, 0] - softmax_outputs[:, 1]).sum().cpu()
                        )
                        class_total[i] += len(labels[labels == i])
                ...
                for ims, labels in tqdm.tqdm(testloader):
                    for i in range(self.num_classes):
                        if len(labels[labels == i]) > 0:
                            ...
                            class_ims += torch.randn_like(class_ims) * noise_std
                            class_outputs = self.net(class_ims)

                            _, class_pred = torch.max(class_outputs, 1)

                            class_correct[i] += class_pred.eq(class_labels).sum().item()
...
def compute_metrics(harness_params):
    ...
    for class_idx in range(harness_params["num_classes"]):
        ...
        if harness_params["compute_decision_boundary_distances"]:
            class_dict[
                f"distance_to_decision_{experiment_seed}"
            ] = decision_boundary_dists[class_idx]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Experiment for Tooling Fairness")

    parser.add_argument(
        "-model_ckpt_path", "--model_ckpt", help="Path to checkpoint", required=True
    )
    parser.add_argument(
        "-results_path",
        "--results_path",
    ...
    parser.add_argument(
        ...
        "--compute_accuracy",
        help="Toggles computation of accuracy metrics",
        action="store_true",
    )
    parser.add_argument(
        "-compute_grad_norms",
        "--compute_grad_norms",
        help="Toggles computation of grad norms",
        action="store_true",
    )
    parser.add_argument(
        "-compute_hessian_eigenvalues",
        "--compute_hessian_eigenvalues",
        help="Toggles computation of hessian eigenvalues",
        action="store_true",
    )
    parser.add_argument(
        "-compute_decision_boundary_distances",
        "--compute_decision_boundary_distances",
        help="Toggles computation of decision boundary distances",
        action="store_true",
    )
    parser.add_argument(
        "-compute_robustness_metrics",
        "--compute_robustness_metrics",
        help="Toggles computation of robustness metrics",
        action="store_true",
    )
    ...
    arg_dict = {
        "dataset": harness_params["dataset"],
        "model": harness_params["model"],
        "test_user_groups": test_user_groups,
        "num_users": num_users,
        "iid": iid,
        "num_classes": num_classes,
        "unequal": unequal,
        "dist_noniid": dist_noniid,
    }
    ...
    for group in range(len(test_num_groups)):
        ...
        my_table.field_names = [
            ...
            "Compute Decision Boundary Distances?",
        ...
        my_table.add_row(
            [
                "FedProx",
                f"{harness_params['model']}",
                f"{harness_params['dataset']}",
                ckpt["seed"],
                harness_params["compute_accuracy"],
                harness_params["compute_grad_norms"],
                harness_params["compute_hessian_eigenvalues"],
                harness_params["compute_decision_boundary_distances"],
                harness_params["compute_robustness_metrics"],
            ]
        )
        print(my_table)
