...
class FLTrainingHarness:
    ...
    def init_global_model(self, model_name, num_classes, num_features=None):
        model_definition = getattr(models, model_name)
    ...
    @param("fl_parameters.fl_method")
    @param("fl_parameters.use_fair_sparsification")
    def train_global_round(
        self, num_clients, frac, fl_method, use_fair_sparsification, epoch
    ):
        ...
        for client_idx in self.idxs_users:
            ...
            if fl_method == "FedSyn":
                ...
                delta, h, w, loss, acc, local_model = self.run_qfedavg_client(
                    local_update=local_update, epoch=epoch
                )
                local_delta_sum += delta
                local_h_sum += h
                local_bitmasks_sum += np.ones_like(local_bitmasks_sum)
            else:
                w, loss, acc, local_model = self.run_generic_client(
                    local_update=local_update, epoch=epoch
                )
    ...
    def compute_client_prob_dist(
        ...
        self.client_prob_dist = {
            ...
            for i, client_idx in enumerate(self.idxs_users)
        }
    ...
    def run_federated_training(self, global_rounds):
        ...
        for epoch in range(global_rounds):
            _, bitmasks, _ = self.train_global_round(epoch=epoch)
